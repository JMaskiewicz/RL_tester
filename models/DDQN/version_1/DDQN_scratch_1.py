...
class DDQN_Agent:
    def __init__(self, input_dims, n_actions, gamma=0.99, alpha=0.001, epsilon=1.0, epsilon_dec=1e-5, epsilon_end=0.01, mem_size=100000, batch_size=64, replace=1000):
        self.gamma = gamma
    ...
    def learn(self):
        ...
        q_pred = self.q_policy(torch.tensor(states, dtype=torch.float).to(self.device))[indices, actions]
        q_next = self.q_target(torch.tensor(states_, dtype=torch.float).to(self.device)).detach()
        q_eval = self.q_policy(torch.tensor(states_, dtype=torch.float).to(self.device)).detach()

        max_actions = torch.argmax(q_eval, dim=1)
        q_next[dones] = 0.0
        q_target = torch.tensor(rewards, dtype=torch.float).to(self.device) + self.gamma * q_next[indices, max_actions]
...
agent = DDQN_Agent(input_dims=env.observation_space.shape,
