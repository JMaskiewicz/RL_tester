...
class TransformerDuelingDDQN(nn.Module):
    def __init__(self, n_actions, input_dims, static_input_dims, n_heads=4, n_layers=2, dropout_rate=1 / 8):
        super(TransformerDuelingDDQN, self).__init__()

        self.transformer_encoder_layer = TransformerEncoderLayer(d_model=input_dims, nhead=n_heads,
                                                                 dropout=dropout_rate, batch_first=True)
        self.transformer_encoder = TransformerEncoder(encoder_layer=self.transformer_encoder_layer, num_layers=n_layers)

        self.positional_encoding = nn.Parameter(torch.zeros(1, 128, input_dims))
        self.fc_static = nn.Linear(static_input_dims, input_dims)

        self.fc1 = nn.Linear(input_dims * 2, 512)  # Adjusted to combine both streams
    ...
    def forward(self, dynamic_state, static_state):
        ...
        val = F.relu(self.fc_value(combined_features))
        val = self.value(val)

        adv = F.relu(self.fc_advantage(combined_features))
...
class DDQN_Agent:
    def __init__(self, input_dims, n_actions, n_epochs=1, mini_batch_size=256, gamma=0.99, policy_alpha=0.001, target_alpha=0.0005, epsilon=1.0, epsilon_dec=1e-5, epsilon_end=0.01, mem_size=100000, batch_size=64, replace=1000, weight_decay=0.0005, l1_lambda=1e-5, static_input_dims=1):
        ...
        self.q_policy = TransformerDuelingDDQN(input_dims, static_input_dims, n_actions).to(self.device)
        self.q_target = TransformerDuelingDDQN(input_dims, static_input_dims, n_actions).to(self.device)
