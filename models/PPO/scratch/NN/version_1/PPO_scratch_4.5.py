...
class PPO_Agent:
    def __init__(self, n_actions, input_dims, gamma=0.95, alpha=0.001, gae_lambda=0.9, policy_clip=0.3, batch_size=1024, n_epochs=20):
...
agent = PPO_Agent(n_actions=env.action_space.n, input_dims=env.calculate_input_dims(), batch_size=len(df_train)-look_back-2, n_epochs=1)
...
train_env = Trading_Environment_Basic(df_train, look_back=20, variables=variables, current_positions=True, tradble_markets=tradable_markets)
...
# Usage Example
PPO_strategy = Simple_PPO(currencies=tradable_markets, df=df_train, agent=agent, look_back=look_back, use_best_action=True, starting_capital=10000, provision=provision)
df_train = PPO_strategy.backtest(df_train)

report_df_positions = PPO_strategy.generate_report(df_train)
report = PPO_strategy.generate_extended_report()
PPO_strategy.display_summary_as_table(df_train, extended_report=True)

# Plotting
from backtest.plots.plot import plot_financial_data

plot_financial_data(df_train, PPO_strategy, tradable_markets, volatility='garman_klass_volatility', n=50)
