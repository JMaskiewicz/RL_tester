...
class Trading_Environment_Basic(gym.Env):
    def __init__(self, df, look_back=20, variables=None, current_positions=True, tradble_markets='EURUSD', provision=0.0001, initial_balance=10000):
...
agent = PPO_Agent(n_actions=env.action_space.n, input_dims=env.calculate_input_dims(), batch_size=len(df_train) - look_back - 1, n_epochs=1)
...
df_test_with_predictions['PnL'] = 0.0
df_test_with_predictions['Provision'] = 0.0
df_test_with_predictions['Equity'] = initial_balance

current_balance = initial_balance
current_position = 0
provision_rate = 0.0001

for index in range(look_back, len(df_test_with_predictions) - 1):
    row = df_test_with_predictions.iloc[index]
    action = row['Predicted_Action'].iloc[0]

    next_row = df_test_with_predictions.iloc[index + 1]
    current_price = row[('Close', tradable_markets)]
    next_price = next_row[('Close', tradable_markets)]
    ...
    if action != current_position:
        if abs(current_position - action) == 2:
            provision_cost = 2 * provision_rate * current_balance
        else:
            provision_cost = provision_rate * current_balance
        current_balance -= provision_cost

    if action != 0:
        pnl = log_return * current_balance * action
        current_balance += pnl

    current_position = action

    # Update the DataFrame
    df_test_with_predictions.at[df_test_with_predictions.index[index], 'PnL'] = pnl
    df_test_with_predictions.at[df_test_with_predictions.index[index], 'Provision'] = provision_cost
    df_test_with_predictions.at[df_test_with_predictions.index[index], 'Equity'] = current_balance

# Print final values
final_row = df_test_with_predictions.iloc[-1]
final_pnl = final_row['PnL']
final_provision = final_row['Provision']
