...
def generate_sinusoidal_data(currencies, start_date, end_date, period_years=3, noise_level=0.01):
    ...
    for currency in currencies:
        ...
        temp_df = pd.DataFrame({
            ('Open', currency): noisy_sinusoidal_values,
            ('High', currency): noisy_sinusoidal_values + 1.05 * noise,  # Adding noise to High
            ('Low', currency): noisy_sinusoidal_values - 1.05 * noise,   # Adding noise to Low
            ('Close', currency): noisy_sinusoidal_values + noise       # Adding noise to Close
        }, index=date_range)
