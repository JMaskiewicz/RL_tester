...
class Transformer_PPO_Agent:
    def __init__(self, n_actions, input_dims, gamma=0.95, alpha=0.001, gae_lambda=0.9, policy_clip=0.2, batch_size=1024,
        ...
        # Learning rate schedulers for the actor and critic optimizers with exponential decay
        ...
        # track the number of generations of the agent
        self.generation = 0
...
def collect_and_learn(dfs, max_episodes_per_worker, env_settings, batch_size_for_learning, backtest_results, agent, num_workers, num_workers_backtesting):
    ...
    shared_episodes_counter = manager.Value('i', 0)  # Shared counter for the number of episodes completed for progress tracking
    ...
    for i in range(num_workers):
        ...
        dfs, shared_queue, max_episodes_per_worker, env_settings, agent, work_event, pause_signals[i],
        resume_signals[i], total_rewards, total_balances, worker_id, individual_worker_batch_size, workers_completed,
        workers_completed_signal, shared_episodes_counter))
    ...
    try:
        ...
        while True:
            ...
            # Progress tracking with controlled update frequency
            current_time = time.time()
            if current_time - last_update_time >= update_interval:
                current_episodes = shared_episodes_counter.value
                progress_percentage = (current_episodes / (max_episodes_per_worker)) * 100
                elapsed_time = current_time - start_time
                eta = ((elapsed_time / current_episodes) * (max_episodes_per_worker - current_episodes)) if current_episodes > 0 else float('inf')
                print(f"\rProgress: {progress_percentage:.2f}% | ETA: {eta:.2f} seconds\n", end='')
        ...
        print("\r" + " " * 80, end='')
        print(f"\rProgress: 100.00% | All episodes processed. Total time: {time.time() - start_time:.2f} seconds.")
