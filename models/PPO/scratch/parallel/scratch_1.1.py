...
def collect_and_learn(dfs, max_episodes_per_worker, env_settings, batch_size_for_learning, backtest_results, agent, num_workers, num_workers_backtesting):
    ...
    try:
        ...
        while True:
            ...
            # Print progress update with custom progress bar
            current_episodes = shared_episodes_counter.value
            progress_percentage = (current_episodes / max_episodes_per_worker * num_workers) * 100
            bar_length = 20
            filled_length = int(round(bar_length * current_episodes / float(max_episodes_per_worker * num_workers)))
            bar = '█' * filled_length + '-' * (bar_length - filled_length)
            elapsed_time = time.time() - start_time
            if elapsed_time > 0:
                speed = current_episodes / elapsed_time
            else:
                speed = float('inf')
            sys.stdout.write(
                f"\033[92m\rProgress: {progress_percentage:.0f}% |{bar}| {current_episodes}/{max_episodes_per_worker * num_workers} [{elapsed_time:.2f}s < {elapsed_time / (progress_percentage / 100) - elapsed_time:.2f}s, {speed:.2f}it/s]\033[0m")
            sys.stdout.flush()
        ...
        print("\nKeyboard interrupt, stopping workers...")

    finally:
        for worker in workers:
            worker.join()  # Ensure all worker processes are properly terminated
        # Clear the line and print the final progress update
