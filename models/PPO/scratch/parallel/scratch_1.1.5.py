"""
Transformer based PPO agent for trading version 3.1
...
def environment_worker(dfs, shared_queue, max_episodes_per_worker, env_settings, agent, work_event, pause_signal, resume_signal, total_rewards, total_balances, worker_id, individual_worker_batch_size, workers_completed, workers_completed_signal):
...
def collect_and_learn(dfs, max_episodes_per_worker, env_settings, batch_size_for_learning, backtestin_results, agent, num_workers, num_workers_backtesting):
    manager = Manager()
    total_rewards = manager.list()
    total_balances = manager.list()
    probs_dfs = manager.dict()
    balances_dfs = manager.dict()
    work_event = Event()
    ...
    agent_generation = 0
    workers_completed = manager.Value('i', 0)  # Track the number of workers that have completed their tasks

    backtesting_completed = Event()
    backtesting_completed.set()  # Initially indicate no backtesting is in progress

    individual_worker_batch_size = batch_size_for_learning // num_workers
    print(f"Starting {num_workers} workers with individual batch size of {individual_worker_batch_size}.")

    workers = []
    for i in range(num_workers):
        worker_id = i + 1
        worker_process = Process(target=environment_worker, args=(
            dfs, shared_queue, max_episodes_per_worker, env_settings, agent, work_event, pause_signals[i], resume_signals[i], total_rewards, total_balances, worker_id, individual_worker_batch_size, workers_completed, workers_completed_signal))
    ...
    try:
        ...
        while True:
            ...
            all_workers_finished = workers_completed.value == num_workers

            if total_experiences >= batch_size_for_learning and backtesting_completed.is_set() and not all_workers_finished:
                backtesting_completed.wait()  # Ensure any ongoing backtesting finishes
                print("Learning phase initiated.")
                agent.learn()
                total_experiences = 0  # Reset experience counter after learning
                ...
                if agent.generation > agent_generation:
                    backtesting_completed.clear()
                    backtest_thread = Thread(target=backtest_in_background, args=(
                        agent, backtest_results, num_workers_backtesting, val_rolling_datasets, test_rolling_datasets, val_labels, test_labels, probs_dfs, balances_dfs, backtesting_completed))
                    backtest_thread.start()
                    agent_generation = agent.generation

            if all_workers_finished:
                # Ensure backtesting is completed before shutting down
                backtesting_completed.wait()
                break

    except KeyboardInterrupt:
        print("Keyboard interrupt, stopping workers...")

    finally:
        for worker in workers:
            worker.join()  # Ensure all worker processes are properly terminated
...
if __name__ == '__main__':
    ...
    collect_and_learn(rolling_datasets, max_episodes_per_worker, env_settings, batch_size, backtest_results, agent, num_workers, num_workers_backtesting)
    ...
    generations_with_all_balances_above_10400 = backtest_results.groupby(level=0).filter(
        lambda x: (x['Final Balance'] > 10400).all()).index.unique()

    print(generations_with_all_balances_above_10400)
