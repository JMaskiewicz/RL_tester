...
class ActorNetwork(nn.Module):
    def __init__(self, n_actions, input_dims, n_heads=4, n_layers=2, dropout_rate=1/5, static_input_dims=1):
...
class CriticNetwork(nn.Module):
    def __init__(self, input_dims, n_heads=4, n_layers=2, dropout_rate=1 / 5, static_input_dims=1):
...
if __name__ == '__main__':
    ...
    for move_forward in range(1, 6):
        ...
        agent = PPO_Agent_T_1D_EURUSD(n_actions=3,  # sell, hold money, buy
                                      ...
                                      alpha=0.00035,  # learning rate for networks (actor and critic) high as its decaying at least 0.0001
        ...
        test_set = backtest_results[backtest_results['Label'] == test_labels[0]]

        # Extract the Sharpe Ratio column for the validation and test sets
        sharpe_ratios_validation = validation_set[(agent.get_name(), 'Sharpe Ratio')]
        sharpe_ratios_test = test_set[(agent.get_name(), 'Sharpe Ratio')]

        # Combine the validation and test Sharpe Ratios into a single DataFrame
        combined_sharpe_ratios = pd.concat([sharpe_ratios_validation, sharpe_ratios_test])

        # Calculate the average Sharpe Ratio for each generation
        average_sharpe_ratios = combined_sharpe_ratios.groupby(combined_sharpe_ratios.index).mean()

        # Find the index of the generation with the highest average Sharpe Ratio
        best_sharpe_index = average_sharpe_ratios.idxmax()
