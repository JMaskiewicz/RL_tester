    ...
    def __call__(self, df, current_index, currencies):
        ...
        for currency in currencies:
            ...
            if currency in self.open_positions:
                # Check for RSI closure
                if (rsi_value > 30 and self.open_positions[currency] > 0) or \
                        (rsi_value < 70 and self.open_positions[currency] < 0):
...
# code
RSI_length = 21
currencies = ['EURUSD']  # You can add more currencies here 'EURJPY'
...
rsi_strategy = SimpleRSIStrategy(RSI_length=RSI_length)
