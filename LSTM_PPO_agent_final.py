...
class PPO_Agent:
    ...
    def learn(self):
        ...
        for _ in range(self.n_epochs):
            state_arr, action_arr, old_prob_arr, vals_arr, reward_arr, dones_arr, static_inputs_arr, batches = self.memory.generate_batches()
            ...
            for start_idx in range(0, len(state_arr), self.mini_batch_size):
                ...
                batch_states = state_arr[minibatch_indices].to(self.device)
                batch_actions = action_arr[minibatch_indices].to(self.device)
                batch_old_probs = old_prob_arr[minibatch_indices].to(self.device)
                batch_advantages = advantages[minibatch_indices].to(self.device)
                batch_returns = discounted_rewards[minibatch_indices].to(self.device)
                batch_static_inputs = static_inputs_arr[minibatch_indices].to(self.device)
...
for episode in tqdm(range(num_episodes)):
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        validation_future = executor.submit(backtest_wrapper, df_validation, agent, 'EURUSD', look_back, variables,provision, starting_balance, leverage)
        test_future = executor.submit(backtest_wrapper, df_test, agent, 'EURUSD', look_back, variables, provision, starting_balance, leverage)
