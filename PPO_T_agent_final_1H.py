...
def make_predictions_AC(df, environment_class, agent, look_back, variables, tradable_markets, provision,
                        starting_balance, leverage):
    predictions_df = pd.DataFrame(index=df.index, columns=['Predicted_Action'])
    env = environment_class(df, look_back=look_back, variables=variables, tradable_markets=tradable_markets,
                            provision=provision, initial_balance=starting_balance, leverage=leverage)
...
def calculate_probabilities_AC(df, environment_class, agent, look_back, variables, tradable_markets, provision,
                               starting_balance,
                               leverage):  # TODO merge with and add atributes to make_predictions_AC, getattr(agent, "choose_best_action")
...
def process_dataset_AC(df, environment_class, agent, look_back, variables, tradable_markets, provision,
                       starting_balance, leverage):
    """
    # TODO add description
    """
    predictions = make_predictions_AC(df, environment_class, agent, look_back, variables, tradable_markets, provision,
                                      starting_balance, leverage)
    probabilities = calculate_probabilities_AC(df, environment_class, agent, look_back, variables, tradable_markets,
                                               provision, starting_balance, leverage)
    return predictions, probabilities


def calculate_probabilities_wrapper_AC(df, environment_class, agent, look_back, variables, tradable_markets, provision,
                                       starting_balance, leverage):
    """
    # TODO add description
    """
    return calculate_probabilities_AC(df, environment_class, agent, look_back, variables, tradable_markets, provision,
                                      starting_balance, leverage)


def generate_predictions_and_backtest_AC(df, agent, mkf, look_back, variables, provision=0.0001, initial_balance=10000,
                                         leverage=1, reward_scaling=1, Trading_Environment_Basic=None):
    """
    ...
    with torch.no_grad():  # Disable gradient computation for inference
        df_with_predictions = make_predictions_AC(df, Trading_Environment_Basic, agent, look_back, variables, mkf,
                                                  provision, initial_balance, leverage)
...
def backtest_wrapper_AC(df, agent, mkf, look_back, variables, provision, initial_balance, leverage, reward_scaling,
    ...
    return generate_predictions_and_backtest_AC(df, agent, mkf, look_back, variables, provision, initial_balance,
                                                leverage, reward_scaling, Trading_Environment_Basic)
...
class Trading_Environment_Basic(gym.Env):
    def __init__(self, df, look_back=20, variables=None, tradable_markets='EURUSD', provision=0.0001,
                 initial_balance=10000, leverage=1, reward_scaling=1):
...
if __name__ == '__main__':
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            validation_future = executor.submit(backtest_wrapper_AC, df_validation, agent, 'EURUSD', look_back,
                                                variables, provision, starting_balance, leverage, reward_scaling,
                                                Trading_Environment_Basic=Trading_Environment_Basic)
            test_future = executor.submit(backtest_wrapper_AC, df_test, agent, 'EURUSD', look_back, variables,
                                          provision, starting_balance, leverage, reward_scaling,
                                          Trading_Environment_Basic=Trading_Environment_Basic)
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            future_train = executor.submit(calculate_probabilities_wrapper_AC, df_train, Trading_Environment_Basic,
                                           agent, look_back, variables, tradable_markets, provision, starting_balance,
                                           leverage)
            future_validation = executor.submit(calculate_probabilities_wrapper_AC, df_validation,
                                                Trading_Environment_Basic, agent, look_back, variables,
                                                tradable_markets, provision, starting_balance, leverage)
            future_test = executor.submit(calculate_probabilities_wrapper_AC, df_test, Trading_Environment_Basic, agent,
        ...
        episode_probabilities['validation'].append(
            validation_probs[['Short', 'Neutral', 'Long']].to_dict(orient='list'))
    ...
    with ThreadPoolExecutor() as executor:
        ...
        for df in datasets:
            futures.append(
                executor.submit(process_dataset_AC, df, Trading_Environment_Basic, agent, look_back, variables,
                                tradable_markets, provision, starting_balance, leverage))
